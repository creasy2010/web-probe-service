 ...
 * @Date    2022/10/5
 **/
export class ZTeamWorkTimeRecord {

    static build(httpClient: IHttpClient) {
        return new ZTeamWorkTimeRecord(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZTeamWorkTimeRecord): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        //@ts-ignore
        if (colInfo["offDate"] && colInfo["offDate"].toLocaleString) {
            //@ts-ignore
            colInfo["offDate"] = `${colInfo["offDate"].getFullYear()}-${colInfo["offDate"].getMonth() + 1}-${colInfo["offDate"].getDate()} ${colInfo["offDate"].getHours()}:${colInfo["offDate"].getMinutes()}:${colInfo["offDate"].getSeconds()}`;
        }


        //@ts-ignore
        if (colInfo["onDate"] && colInfo["onDate"].toLocaleString) {
            //@ts-ignore
            colInfo["onDate"] = `${colInfo["onDate"].getFullYear()}-${colInfo["onDate"].getMonth() + 1}-${colInfo["onDate"].getDate()} ${colInfo["onDate"].getHours()}:${colInfo["onDate"].getMinutes()}:${colInfo["onDate"].getSeconds()}`;
        }
        ...
        return result.ZTeamWorkTimeRecord as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZTeamWorkTimeRecord[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        colInfo = colInfo.map(item => {
            //@ts-ignore
            if (item["offDate"] && item["offDate"].toLocaleString) {
                //@ts-ignore
                item["offDate"] = `${item["offDate"].getFullYear()}-${item["offDate"].getMonth() + 1}-${item["offDate"].getDate()} ${item["offDate"].getHours()}:${item["offDate"].getMinutes()}:${item["offDate"].getSeconds()}`;
            }
        ...
        colInfo = colInfo.map(item => {
            //@ts-ignore
            if (item["onDate"] && item["onDate"].toLocaleString) {
                //@ts-ignore
                item["onDate"] = `${item["onDate"].getFullYear()}-${item["onDate"].getMonth() + 1}-${item["onDate"].getDate()} ${item["onDate"].getHours()}:${item["onDate"].getMinutes()}:${item["onDate"].getSeconds()}`;
            }
     ...
     * @param id
     */
    async get(id: number): Promise<IZTeamWorkTimeRecord> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZTeamWorkTimeRecord>): IZTeamWorkTimeRecord {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZTeamWorkTimeRecord>): IZTeamWorkTimeRecord {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZTeamWorkTimeRecord>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZTeamWorkTimeRecord) {
        //slot batchUpdate
        //@ts-ignore
        ...
        //@ts-ignore
        if (modifyColInfo["offDate"] && modifyColInfo["offDate"].toLocaleString) {
            //@ts-ignore
            modifyColInfo["offDate"] = `${modifyColInfo["offDate"].getFullYear()}-${modifyColInfo["offDate"].getMonth() + 1}-${modifyColInfo["offDate"].getDate()} ${modifyColInfo["offDate"].getHours()}:${modifyColInfo["offDate"].getMinutes()}:${modifyColInfo["offDate"].getSeconds()}`;
        }


        //@ts-ignore
        if (modifyColInfo["onDate"] && modifyColInfo["onDate"].toLocaleString) {
            //@ts-ignore
            modifyColInfo["onDate"] = `${modifyColInfo["onDate"].getFullYear()}-${modifyColInfo["onDate"].getMonth() + 1}-${modifyColInfo["onDate"].getDate()} ${modifyColInfo["onDate"].getHours()}:${modifyColInfo["onDate"].getMinutes()}:${modifyColInfo["onDate"].getSeconds()}`;
        }
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZTeamWorkTimeRecord.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZTeamWorkTimeRecord.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZTeamWorkTimeRecord: IZTeamWorkTimeRecord;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZTeamWorkTimeRecord?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
        ...
        result['[]']?.ZTeamWorkTimeRecord?.forEach(item => {
            if (item["offDate"]) {
                //@ts-ignore
                item["offDate"] = new Date(item["offDate"]);
            }
        })
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZTeamWorkTimeRecord[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZTeamWorkTimeRecord) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZTeamWorkTimeRecord {
    id?: number;// 唯一标识:
    userId?: number;// 租户id:
    name?: string;// 姓名:
    workNo?: string;// 工号:
