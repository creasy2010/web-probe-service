 ...
 * @Date    2022/10/5
 **/
export class ZMMResourceUrl {

    static build(httpClient: IHttpClient) {
        return new ZMMResourceUrl(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMResourceUrl): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZMMResourceUrl as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMResourceUrl[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMResourceUrl> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMResourceUrl>): IZMMResourceUrl {
        //slot fromDb
        //@ts-ignore
        if (item["updateDate"] && item["updateDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMResourceUrl>): IZMMResourceUrl {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZMMResourceUrl>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMResourceUrl) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMResourceUrl.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMResourceUrl.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMResourceUrl: IZMMResourceUrl;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMResourceUrl?.forEach(item => {
            if (item["updateDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMResourceUrl[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMResourceUrl) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZMMResourceUrl {
    id?: number;// 唯一标识:
    userId?: number;// 用户id:
    url?: string;// 资源地址:
    host?: string;// 资源域名:
    type?: string;// 资源类型:
    contentSize?: number;// 资源长度:
    contentHash?: string;// 资源内容hash:
    addDate?: Date;// 添加时间:
