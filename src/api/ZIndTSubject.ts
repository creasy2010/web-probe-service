 ...
 * @Date    2022/10/5
 **/
export class ZIndTSubject {

    static build(httpClient: IHttpClient) {
        return new ZIndTSubject(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZIndTSubject): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZIndTSubject as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZIndTSubject[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZIndTSubject> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZIndTSubject>): IZIndTSubject {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZIndTSubject>): IZIndTSubject {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZIndTSubject>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZIndTSubject) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZIndTSubject.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZIndTSubject.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZIndTSubject: IZIndTSubject;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZIndTSubject?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
    ...
    async query(param: IQueryParam): Promise<{
        ...
        return {
            data: result['[]']?.map?.((item: any) => item.ZIndTSubject) || [],
            total
        };
    }
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZIndTSubject[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZIndTSubject) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZIndTSubject {
    id?: number;// 唯一标识:
    userId?: number;// 租户id:
