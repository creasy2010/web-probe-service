 ...
 * @Date    2022/10/5
 **/
export class ZMoonCodeGenRecord {

    static build(httpClient: IHttpClient) {
        return new ZMoonCodeGenRecord(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMoonCodeGenRecord): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZMoonCodeGenRecord as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMoonCodeGenRecord[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZMoonCodeGenRecord> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMoonCodeGenRecord>): IZMoonCodeGenRecord {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMoonCodeGenRecord>): IZMoonCodeGenRecord {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZMoonCodeGenRecord>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMoonCodeGenRecord) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMoonCodeGenRecord.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMoonCodeGenRecord.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMoonCodeGenRecord: IZMoonCodeGenRecord;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMoonCodeGenRecord?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMoonCodeGenRecord[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMoonCodeGenRecord) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZMoonCodeGenRecord {
    id?: number;// 唯一标识:
    userId?: number;// 用户id:
    bffIds?: string;// bffIds:
    dubboIds?: string;// dubboIds:
    count?: number;// 接口数量:
    avgTimeConsume?: number;// 平均花费时间（秒）:
    totalTimeConsume?: number;// 花费时间（秒）:
    warningINfo?: string;// 警告:
