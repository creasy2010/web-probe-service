 ...
 * @Date    2022/10/5
 **/
export class ZMMResourceTab {

    static build(httpClient: IHttpClient) {
        return new ZMMResourceTab(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMResourceTab): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZMMResourceTab as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMResourceTab[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMResourceTab> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMResourceTab>): IZMMResourceTab {
        //slot fromDb
        //@ts-ignore
        if (item["updateDate"] && item["updateDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMResourceTab>): IZMMResourceTab {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZMMResourceTab>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMResourceTab) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMResourceTab.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMResourceTab.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMResourceTab: IZMMResourceTab;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMResourceTab?.forEach(item => {
            if (item["updateDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMResourceTab[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMResourceTab) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZMMResourceTab {
    id?: number;// 唯一标识:
