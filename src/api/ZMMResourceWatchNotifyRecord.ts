...
import {IZMMResourceChangeRecord} from "./ZMMResourceChangeRecord";
 ...
 * @Date    2022/10/5
 **/
export class ZMMResourceWatchNotifyRecord {

    static build(httpClient: IHttpClient) {
        return new ZMMResourceWatchNotifyRecord(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMResourceWatchNotifyRecord): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        if (typeof colInfo["changeRecordIds"] !== 'string' && typeof colInfo["changeRecordIds"] === 'object') {
            colInfo["changeRecordIds"] = JSON.stringify(colInfo["changeRecordIds"]);
        }


        let result = await this.httpClient.post<IResult>(`/post`, {
            "ZMMResourceWatchNotifyRecord": {
                ...colInfo,
            },
            "tag": "ZMMResourceWatchNotifyRecord"
        });
        ...
        return result.ZMMResourceWatchNotifyRecord as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMResourceWatchNotifyRecord[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        let result = await this.httpClient.post<IResult>(`/post`, {
            "ZMMResourceWatchNotifyRecord[]": colInfo,
            "tag": "ZMMResourceWatchNotifyRecord[]"
        });
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMResourceWatchNotifyRecord> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMResourceWatchNotifyRecord>): IZMMResourceWatchNotifyRecord {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMResourceWatchNotifyRecord>): IZMMResourceWatchNotifyRecord {
        //slot toDb
        if (typeof item["changeRecordIds"] !== 'string' && typeof item["changeRecordIds"] === 'object') {
            item["changeRecordIds"] = JSON.stringify(item["changeRecordIds"]);
        }
    ...
    async update(item: Partial<IZMMResourceWatchNotifyRecord>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMResourceWatchNotifyRecord) {
        //slot batchUpdate
        //@ts-ignore
        ...
        if (typeof modifyColInfo["changeRecordIds"] !== 'string' && typeof modifyColInfo["changeRecordIds"] === 'object') {
            modifyColInfo["changeRecordIds"] = JSON.stringify(modifyColInfo["changeRecordIds"]);
        }
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMResourceWatchNotifyRecord.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMResourceWatchNotifyRecord.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMResourceWatchNotifyRecord: IZMMResourceWatchNotifyRecord;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMResourceWatchNotifyRecord?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMResourceWatchNotifyRecord[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMResourceWatchNotifyRecord) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
     ...
     * @param param
     */
    private async executeQueryWithRel(param: IQueryParam): Promise<(IZMMResourceWatchNotifyRecord & { changeRecordIds?: IZMMResourceChangeRecord })[]> {
        let result = await this.httpClient.post(`/get`, {
            '[]': {
                page: param?.page || 0,
                ...
                ZMMResourceChangeRecord: {
                    'id@': '/ZMMResourceWatchNotifyRecord/changeRecordIds',
                }
        ...
        for (let i = 0, iLen = result['[]'].length; i < iLen; i++) {
            let item = result['[]'][i] as IZMMResourceWatchNotifyRecord;
            let newItem = {
                ...item.ZMMResourceWatchNotifyRecord,
                changeRecordIds: item.ZMMResourceChangeRecord, _changeRecordIds: item.ZMMResourceChangeRecord
            };
    ...
    async queryWithRel(param: IQueryParam): Promise<{
        data: (IZMMResourceWatchNotifyRecord & { _changeRecordIds?: IZMMResourceChangeRecord })[];
        total: number;
    }> {
    ...
    ): Promise<IZMMResourceWatchNotifyRecord[]> {
        let totalCount = await this.queryTotalCount(condition);
        const perPage = 100;
        let totalPage = Math.ceil(totalCount / perPage);
        let allRequest = [];
...
export interface IZMMResourceWatchNotifyRecord {
    id?: number;// 唯一标识:
    userId?: number;// 用户id:
    changeRecordIds?: any;// 变动记录IDs:
