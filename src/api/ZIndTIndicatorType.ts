 ...
 * @Date    2022/10/5
 **/
export class ZIndTIndicatorType {

    static build(httpClient: IHttpClient) {
        return new ZIndTIndicatorType(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZIndTIndicatorType): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZIndTIndicatorType as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZIndTIndicatorType[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZIndTIndicatorType> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZIndTIndicatorType>): IZIndTIndicatorType {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZIndTIndicatorType>): IZIndTIndicatorType {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZIndTIndicatorType>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZIndTIndicatorType) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZIndTIndicatorType.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZIndTIndicatorType.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZIndTIndicatorType: IZIndTIndicatorType;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZIndTIndicatorType?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZIndTIndicatorType[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZIndTIndicatorType) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZIndTIndicatorType {
    id?: number;// 唯一标识:
    userId?: number;// 租户id:
