 ...
 * @Date    2022/10/5
 **/
export class ZMMResourceMonitorUrl {

    static build(httpClient: IHttpClient) {
        return new ZMMResourceMonitorUrl(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMResourceMonitorUrl): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        //@ts-ignore
        if (colInfo["lastCollectDate"] && colInfo["lastCollectDate"].toLocaleString) {
            //@ts-ignore
            colInfo["lastCollectDate"] = `${colInfo["lastCollectDate"].getFullYear()}-${colInfo["lastCollectDate"].getMonth() + 1}-${colInfo["lastCollectDate"].getDate()} ${colInfo["lastCollectDate"].getHours()}:${colInfo["lastCollectDate"].getMinutes()}:${colInfo["lastCollectDate"].getSeconds()}`;
        }


        if (typeof colInfo["resInfo"] !== 'string' && typeof colInfo["resInfo"] === 'object') {
            colInfo["resInfo"] = JSON.stringify(colInfo["resInfo"]);
        }


        let result = await this.httpClient.post<IResult>(`/post`, {
            "ZMMResourceMonitorUrl": {
                ...colInfo,
            },
            "tag": "ZMMResourceMonitorUrl"
        });
        ...
        return result.ZMMResourceMonitorUrl as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMResourceMonitorUrl[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        colInfo = colInfo.map(item => {
            //@ts-ignore
            if (item["lastCollectDate"] && item["lastCollectDate"].toLocaleString) {
                //@ts-ignore
                item["lastCollectDate"] = `${item["lastCollectDate"].getFullYear()}-${item["lastCollectDate"].getMonth() + 1}-${item["lastCollectDate"].getDate()} ${item["lastCollectDate"].getHours()}:${item["lastCollectDate"].getMinutes()}:${item["lastCollectDate"].getSeconds()}`;
            }
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMResourceMonitorUrl> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMResourceMonitorUrl>): IZMMResourceMonitorUrl {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMResourceMonitorUrl>): IZMMResourceMonitorUrl {
        //slot toDb
        if (typeof item["resInfo"] !== 'string' && typeof item["resInfo"] === 'object') {
            item["resInfo"] = JSON.stringify(item["resInfo"]);
        }
    ...
    async update(item: Partial<IZMMResourceMonitorUrl>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMResourceMonitorUrl) {
        //slot batchUpdate
        //@ts-ignore
        ...
        //@ts-ignore
        if (modifyColInfo["lastCollectDate"] && modifyColInfo["lastCollectDate"].toLocaleString) {
            //@ts-ignore
            modifyColInfo["lastCollectDate"] = `${modifyColInfo["lastCollectDate"].getFullYear()}-${modifyColInfo["lastCollectDate"].getMonth() + 1}-${modifyColInfo["lastCollectDate"].getDate()} ${modifyColInfo["lastCollectDate"].getHours()}:${modifyColInfo["lastCollectDate"].getMinutes()}:${modifyColInfo["lastCollectDate"].getSeconds()}`;
        }


        if (typeof modifyColInfo["resInfo"] !== 'string' && typeof modifyColInfo["resInfo"] === 'object') {
            modifyColInfo["resInfo"] = JSON.stringify(modifyColInfo["resInfo"]);
        }
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMResourceMonitorUrl.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMResourceMonitorUrl.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMResourceMonitorUrl: IZMMResourceMonitorUrl;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMResourceMonitorUrl?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
        ...
        result['[]']?.ZMMResourceMonitorUrl?.forEach(item => {
            if (item["lastCollectDate"]) {
                //@ts-ignore
                item["lastCollectDate"] = new Date(item["lastCollectDate"]);
            }
        })
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMResourceMonitorUrl[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMResourceMonitorUrl) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
     ...
     * @param param
     */
    private async executeQueryWithRel(param: IQueryParam): Promise<(IZMMResourceMonitorUrl & { resourceInfo?: IZMMResourceUrl })[]> {
        let result = await this.httpClient.post(`/get`, {
            '[]': {
                page: param?.page || 0,
                ...
                ZMMResourceUrl: {
                    'id@': '/ZMMResourceMonitorUrl/resourceId',
                }
        ...
        for (let i = 0, iLen = result['[]'].length; i < iLen; i++) {
            let item = result['[]'][i] as IZMMResourceMonitorUrl;
            let newItem = {
                ...item.ZMMResourceMonitorUrl,
    ...
    async queryWithRel(param: IQueryParam): Promise<{
        data: (IZMMResourceMonitorUrl & { _resourceInfo?: IZMMResourceUrl })[];
        total: number;
    }> {
    ...
    ): Promise<IZMMResourceMonitorUrl[]> {
        let totalCount = await this.queryTotalCount(condition);
        const perPage = 100;
        let totalPage = Math.ceil(totalCount / perPage);
        let allRequest = [];
...
export interface IZMMResourceMonitorUrl {
    id?: number;// 唯一标识:
    userId?: number;// 用户id:
    resourceId?: number;// 待监控资源id:
    resInfo?: any;// 暂不使用:{type:xxx,info:any},type:resource,info:id;
    lastCollectDate?: Date;// 上次采集时间:
