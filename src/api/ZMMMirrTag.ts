 ...
 * @Date    2022/10/5
 **/
export class ZMMMirrTag {

    static build(httpClient: IHttpClient) {
        return new ZMMMirrTag(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMMirrTag): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZMMMirrTag as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMMirrTag[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMMirrTag> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMMirrTag>): IZMMMirrTag {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMMirrTag>): IZMMMirrTag {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZMMMirrTag>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMMirrTag) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMMirrTag.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMMirrTag.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMMirrTag: IZMMMirrTag;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMMirrTag?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
    ...
    async query(param: IQueryParam): Promise<{
        ...
        return {
            data: result['[]']?.map?.((item: any) => item.ZMMMirrTag) || [],
            total
        };
    }
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMMirrTag[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMMirrTag) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
