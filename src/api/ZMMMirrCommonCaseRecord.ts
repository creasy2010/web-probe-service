 ...
 * @Date    2022/10/5
 **/
export class ZMMMirrCommonCaseRecord {

    static build(httpClient: IHttpClient) {
        return new ZMMMirrCommonCaseRecord(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZMMMirrCommonCaseRecord): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        let result = await this.httpClient.post<IResult>(`/post`, {
            "ZMMMirrCommonCaseRecord": {
                ...colInfo,
            },
            "tag": "ZMMMirrCommonCaseRecord"
        });
        ...
        return result.ZMMMirrCommonCaseRecord as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZMMMirrCommonCaseRecord[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        let result = await this.httpClient.post<IResult>(`/post`, {
            "ZMMMirrCommonCaseRecord[]": colInfo,
            "tag": "ZMMMirrCommonCaseRecord[]"
        });
     ...
     * @param id
     */
    async get(id: number): Promise<IZMMMirrCommonCaseRecord> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZMMMirrCommonCaseRecord>): IZMMMirrCommonCaseRecord {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZMMMirrCommonCaseRecord>): IZMMMirrCommonCaseRecord {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZMMMirrCommonCaseRecord>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZMMMirrCommonCaseRecord) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZMMMirrCommonCaseRecord.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZMMMirrCommonCaseRecord.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZMMMirrCommonCaseRecord: IZMMMirrCommonCaseRecord;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZMMMirrCommonCaseRecord?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZMMMirrCommonCaseRecord[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZMMMirrCommonCaseRecord) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZMMMirrCommonCaseRecord {
    id?: number;// 唯一标识:
