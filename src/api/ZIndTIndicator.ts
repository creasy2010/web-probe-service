 ...
 * @Date    2022/10/5
 **/
export class ZIndTIndicator {

    static build(httpClient: IHttpClient) {
        return new ZIndTIndicator(httpClient);
    }
     ...
     * @param colInfo
     */
    async add(colInfo: IZIndTIndicator): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
        ...
        return result.ZIndTIndicator as any;
    }
     ...
     * @param colInfo
     */
    async addBatch(colInfo: IZIndTIndicator[]): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param id
     */
    async get(id: number): Promise<IZIndTIndicator> {
     ...
     * @param item
     */
    fromDb(item: Partial<IZIndTIndicator>): IZIndTIndicator {
        //slot fromDb
        //@ts-ignore
        if (item["addDate"] && item["addDate"].toLocaleString) {
            //@ts-ignore
     ...
     * @param item
     */
    toDb(item: Partial<IZIndTIndicator>): IZIndTIndicator {
        //slot toDb
        return item as any;
    }


    async update(item: Partial<IZIndTIndicator>): Promise<{
        ok: boolean;
        id: number;
        count: number;
    }> {
     ...
     * @param modifyColInfo
     */
    async updateBatch(ids: number[], modifyColInfo: IZIndTIndicator) {
        //slot batchUpdate
        //@ts-ignore
    ...
    async deleteByCondition(condition: IQueryColCondition): Promise<number> {
        ...
        return result.ZIndTIndicator.count;
    }
    ...
    async countByCol(condition: IQueryColCondition) {
        ...
        return result.ZIndTIndicator.count as any;

    }

    private async executeQuery(param: IQueryParam): Promise<{
        '[]': {
            ZIndTIndicator: IZIndTIndicator;
            [key: string]: any
        }
    }[]> {
        //slot executeQuery
        ...
        result['[]']?.ZIndTIndicator?.forEach(item => {
            if (item["addDate"]) {
                //@ts-ignore
     ...
     *
     */
    async queryAll(condition: IQueryColCondition): Promise<IZIndTIndicator[]> {
        ...
        return allResult.map((result: any) => result['[]']?.map?.((item: any) => item.ZIndTIndicator) || [])?.reduce((pre, cur) => pre.concat(cur), []) || [];
    }
...
export interface IZIndTIndicator {
    id?: number;// 唯一标识:
    userId?: number;// 租户id:
